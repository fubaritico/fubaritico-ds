...
## ⚠️ Project reconciliation note (fubaritico-ds)

This project also ships ` audit-style` (flat-selector BEM). The two skills diverge on nesting:
`bem-structure` allows `&`-nested CSS; ` audit-style` mandates **flat selectors**. For this
project's **override-first / white-label** goal, prefer **flat, single-class selectors**
(`.ui-badge__icon {}`) over nesting — nested CSS (`.card .card__title`) raises specificity and
fights consumer overrides. Author the skin inside `@layer` so unlayered consumer selectors win
without `!important`. Keep BEM naming from this skill; keep flatness from ` audit-style`.
